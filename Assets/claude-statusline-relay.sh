...
# is omitted, never NaN or a guessed zero. Half ties round to even, matching
# CompactLiveCopy.percent. jq and JXA must not drift.
if command -v jq >/dev/null 2>&1; then
  printf '%s' "$input" | jq -r 'def whole: floor as $f | if . - $f == 0.5 then $f + ($f % 2) else . + 0.5 | floor end;
    [(.model.display_name // .model.id // empty),
    (.context_window.used_percentage | select(. != null) | "ctx \(floor)%"),
    (.rate_limits.five_hour.used_percentage | select(. != null) | "Claude 5h \([0, 100 - .] | max | whole)% left")]
    | join(" · ")' 2>/dev/null
  ...
  TOKEN_BAR_STATUS_JSON="$input" osascript -l JavaScript -e '
    ...
    const whole = x => { const f = Math.floor(x); return x - f === 0.5 ? f + (f % 2) : Math.round(x); };
    const remaining = used == null ? null : whole(Math.max(0, 100 - used));
