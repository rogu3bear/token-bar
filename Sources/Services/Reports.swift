...
struct UsageReport {
    ...
    func csv() -> String {
        ...
        let rows = entries.map { e in
            [iso.string(from: e.date), e.session, e.model, String(e.tokens.input), String(e.tokens.cached), String(e.tokens.output), String(e.tokens.reasoning), String(e.tokens.total), e.account?.label ?? DimensionReport.unattributed, UsageMetadata.accountAttribution(e.account), e.bucket ?? "event", String(e.eventCount)].map(quote).joined(separator: ",")
        }
