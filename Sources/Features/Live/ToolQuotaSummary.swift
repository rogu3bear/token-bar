        ...
        VStack(alignment: .leading, spacing: 10) {
            ...
            if let reading {
                if !detailsOnly {
                    ...
                    Text("\(tool.label) · \(reading.windowLabel) window" + (reading.resetWhen.map { " · resets " + $0 } ?? " · reset unavailable"))
                        .font(.caption).foregroundStyle(.secondary)
                }
        ...
        DisclosureGroup(isExpanded: $expanded) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                ...
                if let reading = allowance.reading {
                    Text(reading.windowLabel + (reading.resetWhen.map { " · resets " + $0 } ?? " · reset unavailable"))
                        .font(.caption).foregroundStyle(.secondary)
