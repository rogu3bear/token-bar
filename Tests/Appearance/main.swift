...
let acknowledgements = NoticeDismissals()
let diagnosticKey = NoticeDismissals.key(["Codex|history|file|gap", "Claude|provider|read failure"])
acknowledgements.dismiss(diagnosticKey)
assert(acknowledgements.contains(NoticeDismissals.key(["Claude|provider|read failure", "Codex|history|file|gap"])))
assert(!acknowledgements.contains(NoticeDismissals.key(["Codex|history|file|gap", "Grok|provider|read failure"])))
assert(!NoticeDismissals().contains(diagnosticKey), "Acknowledgements must not survive a new app session")
let periodReset = PreviewFixture.date.addingTimeInterval(300)
acknowledgements.dismissAllowance("account/window", reset: periodReset, level: 1)
assert(acknowledgements.containsAllowance("account/window", reset: periodReset.addingTimeInterval(45), level: 1))
assert(!acknowledgements.containsAllowance("account/window", reset: periodReset.addingTimeInterval(90), level: 1), "Reset jitter is compared to the fixed acknowledged anchor")
assert(!acknowledgements.containsAllowance("account/window", reset: periodReset, level: 2), "Escalation must return")
assert(!acknowledgements.containsAllowance("different account/window", reset: periodReset, level: 1))
assert(!acknowledgements.containsAllowance("account/window", reset: nil, level: 1))
acknowledgements.dismissAllowance("without reset", reset: nil, level: 1)
assert(acknowledgements.containsAllowance("without reset", reset: nil, level: 1))
assert(!acknowledgements.containsAllowance("without reset", reset: periodReset, level: 1))
acknowledgements.restore()
assert(!acknowledgements.hasDismissed)
assert(!acknowledgements.containsAllowance("without reset", reset: nil, level: 1))
assert(retainedGaps.readHealth.continuityCount == 2359 && retainedGaps.readHealth.partial)
print("PASS: warning acknowledgement is session-only, preserves diagnostics, survives stable observations, isolates account/window, and returns on escalation or a new reset")
