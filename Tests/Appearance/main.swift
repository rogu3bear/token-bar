...
assert(Destination.capsule == [.now, .history, .cost, .accounts, .insights])
assert(!Destination.capsule.contains(.menuBar) && !Destination.capsule.contains(.appearance))
assert(Destination.settings == [.menuBar, .appearance])
print("PASS: capsule membership is the five product destinations")
