...
- After an end-to-end stills pass: Guard chrome only when a warning exists; idle Now and the popover keep unconfirmed chairs for stale or failed remaining; Allowances does not reprint Codex remaining under the disclosures; Settings Rate units stays a readable label; welcome and empty Allowances name all three CLIs.
